...
- `build`/`revise` modes are **enqueued to the real worker** (`ai-generate-worker`, `{ timeout: 330000, workers: 1 }`). POST returns the jobId instantly; the frontend polls (`aiRequest` in `shell/api.js`, 360s cap). Timeout ordering contract: build budget < worker timeout (330s) < frontend poll deadline (360s) < job TTL (10min). Per the Codehooks docs, paid plans allow worker timeouts up to **10 minutes** — an earlier "PRO limit is 120s" belief here was wrong and made every nontrivial M3 build die at 110s.
- **Multi-file build (`buildModule`):** three stages, each a bounded call so no single response truncates:
  1. **Architect** (`SYSTEM_PROMPT`, M3): emits `{ manifest, main, files }`. `main` is the entry class; `files` is a list of `{name, exports, spec}` for large apps (the architect keeps `main` a thin orchestrator and splits feature areas into files). A small app returns `files: []` → single-file (`{ js: main }`, the legacy n=1 case).
  2. **Per-feature** (`FEATURE_PROMPT`, same model, **parallel**): each feature file generated against the fixed `manifest + main.js` + its required exports. Feature functions take the module instance (`host`) as first arg; **star topology** — feature files import only `/shell/`, never each other or main (enforced: a relative import in a feature → build rejected, since `assembleModuleBlobs` only wires the entry's relative imports).
  3. **CSS** (`STYLE_PROMPT`, fast model, max_tokens 4096): **one chunk per JS file, parallel** (`generateCss` over the files map) — the entry chunk owns root/layout/theme, feature chunks style only their own classes — so CSS scales with the app instead of one capped call (a single 4096 call would silently truncate → unstyled on large apps). Per-chunk failure is non-fatal (that chunk ships unstyled).
  Result is `{ manifest, files:{...}, entryFile:'main.js', css }` (multi) or `{ manifest, js, css }` (single). Budgets are ceilings (worker ~220s/70s/38s, inline 38s/12s/12s); a multi-file architect returns a thin `main` fast, leaving room for parallel features within the 330s worker / 65s inline-POST window. `result.error` (architect/feature failure) is surfaced verbatim; otherwise `finishReason 'length'` → truncation message.
...
- **Failure telemetry:** every build outcome bumps a durable KV counter via `bumpStat` — `ai_stat:{timeout|truncation|invalid_json|success}` (no ttl, best-effort, races tolerated). Read with `coho` to see the real failure mix.
...
**Models:** `MiniMax-M2.7-highspeed` for clarify/plan (max_tokens 1024/2048) and the separate CSS call (max_tokens 4096); `MiniMax-M3` for the architect + per-feature calls of build/revise (architect max_tokens 32768, feature 16384 — M3's `<think>` reasoning tokens count against max_tokens (~4k typical), so the cap must leave room for reasoning + code. Multi-file splitting keeps each call well under the cap. Runs in the worker, ~1–3min at ~120 tok/s; `extractModuleJson` strips the `<think>` blocks). No `response_format` param — it caused request hangs; JSON extraction is handled by `extractModuleJson` instead.
