...
- **Two-call build (CSS split):** `buildModule()` runs the JS generation (`MiniMax-M3`, ~260s budget) emitting `{manifest, js}` only, then a separate CSS call (`STYLE_PROMPT` on `MiniMax-M2.7-highspeed`, ~45s, max_tokens 4096) that sees the generated js and styles its classes. Splitting CSS out shrinks the JS-call output (CSS is a big share of rich apps) → less truncation, faster JS call; both calls fit the 330s worker. `SYSTEM_PROMPT` therefore outputs **no `css` field**. CSS failure is non-fatal — `generateCss` returns `''` so a working module ships unstyled rather than being lost. Inline fallback uses the same helper (JS ~50s + CSS ~25s on highspeed).
