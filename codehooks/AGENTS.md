...
- **Multi-file build (`buildModule`) — plan-split, every call bounded so none truncates:**
  ...
  4. **CSS** (`STYLE_PROMPT`, fast model, max_tokens 4096): **one chunk per JS file, parallel**, now **narrowed**. The shell ships a styled `c-*` component library (`shell/css/components.css`, adopted into every shadow), so `SYSTEM_PROMPT`/`FEATURE_PROMPT` tell the model to render with those classes and `STYLE_PROMPT` emits only app-specific *layout* (often `{ "css": "" }`). This kills the "empty CSS → unstyled / `/modules/<id>/styles.css` 404" failure (generated apps also always get a cssUrl blob — `_registerModule`/`_loadGeneratedModules`) and cuts CSS-step truncation. Per-chunk failure stays non-fatal — components carry the baseline look.
...
- **Scope-split (always for big apps):** `PLAN_PROMPT` aggressively puts only the essential, usable **core** in `plan.features` and everything else in `plan.deferred` (`[{title,desc}]`). The architect builds only the core; the frontend stores `deferred` on the module and shows one-tap **➕ Add** buttons that seed a scoped Revise (which moves the item into the core, rebuilds via multi-file, keeps appId/tag → data survives, and shrinks the roadmap). **Complex archetypes** (games, editors, spreadsheets, calendars) are explicitly required to ship a tiny core + large deferred list — they fail if built whole.
- **Plan fidelity (`fidelity-v1`):** `PLAN_PROMPT` picks `generator` from one-per-thing / collaborative wording (plural nouns, "one per X", shared/multi-user) instead of defaulting everything to singleton. `REVISE_SUFFIX` requires that any new state field added by a revise is **defaulted in `_load()`** (and backfilled for already-saved data) so a revise can't crash existing installs with `undefined`, and that a revise **keeps prior capabilities** (e.g. an added settings panel).
