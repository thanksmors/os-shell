...
**Taskbar & Spaces menu.** The taskbar holds only the **Spaces button** (Saturn logo, embossed/glassy — `.os-start-btn`), running-window buttons, and the clock. The Spaces button opens the **onboarding modal hub** (`openOnboarding`), a 3-page modal (`index.html`): page 1 ODVI ecosystem apps, page 2 feature tour, **page 3 = embedded Settings** — an icon row (the settings tabs, styled like pages 1–2) where clicking an icon renders that tab's content inline (`.os-ob-settings-body`). The old in-taskbar theme toggle and workspace switcher were removed — Dark mode lives in the Appearance tab, Switch space + Sign out in the Workspace tab. Apps launch from **desktop icons** only. `desktopItems()` excludes apps with `manifest.hidden: true` (e.g. Settings, reached via the hub) in addition to `generator`/`isAppHidden`.

**Embedded settings — `shell/settings-embed.js`.** `renderSettingsTab(tabId, contentEl)` renders a Settings tab into any light-DOM element by reusing the exact tab renderers the Settings *window* uses (`modules/settings/tabs/*`), with a host adapter (`_esc`, `api.store`, `api.notify`, `api.requestClose → closeOnboarding`, `_renderTab`). `SETTINGS_CSS` is injected once into the page `<head>`; the tab markup uses bare `.settings-*` classes so global injection styles it (it can't reach other modules' shadow DOM). The os store exposes `embedSettingsTab(tabId, el)` (lazy-imports the helper) for the hub to call. The standalone Settings window (`modules/settings/index.js`) is unchanged and still launchable.
