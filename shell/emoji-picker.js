...
function _buildPickerHTML(emojis, activeTab, searchQuery) {
  const dark = _isDark();
  ...
  const visibleEmojis = (() => {
    ...
    if (activeTab === 0) {
      return recent.map(char => ({ char, name: char }));
    }
  ...
  const tabs = CATEGORY_MAP.map((cat, i) => `
    <button class="ep-tab ${i === activeTab && !searchQuery ? 'active' : ''}" data-tab="${i}" title="${cat.label}">
      ${cat.icon}
    </button>
  `).join('');

  const grid = visibleEmojis.length === 0
    ? `<div class="ep-empty">${searchQuery ? 'No results' : 'None yet'}</div>`
    : visibleEmojis.map(e => `<button class="ep-emoji" data-emoji="${e.char}" title="${e.name || ''}">${e.char}</button>`).join('');
  ...
  return `
    ...
    <div class="ep-wrap">
      <div class="ep-search-row">
        <input class="ep-search" placeholder="Search emojis…" value="${searchQuery}" autofocus />
      </div>
...
export function showEmojiPicker(anchorEl, currentEmoji, onPick) {
  ...
  function render() {
    container.innerHTML = _buildPickerHTML(emojis, activeTab, searchQuery);
    bindEvents();
    positionPicker();
    if (searchQuery === '' && !_emojiData) {
      container.querySelector('.ep-grid').innerHTML = '<div class="ep-loading">Loading…</div>';
    }
    // Auto-focus search
  ...
  function bindEvents() {
    ...
    // Stop clicks inside picker from bubbling to document dismiss listener
  ...
  if (!_emojiData) {
    _loadEmojis().then(data => {
      emojis = data;
      if (_activePicker === container) render();
    });
  }
}
